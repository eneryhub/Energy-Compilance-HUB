...
// Lazy imports — prevents serverless crash if packages aren't installed on Vercel
...
async function getEmbeddings(text: string) {
  if (!_getEmbeddings) {
    try {
      ...
      console.error('[Paperclip Ingest] Failed to import @/lib/ai — is z-ai-web-dev-sdk installed?', err)
      return null
    }
  }
  return _getEmbeddings(text)
}
...
async function getSupabaseClient() {
  if (!_getSupabaseClient) {
    try {
      ...
      console.error('[Paperclip Ingest] Failed to import @/lib/supabase — is @supabase/supabase-js installed?', err)
      return null
    }
  }
