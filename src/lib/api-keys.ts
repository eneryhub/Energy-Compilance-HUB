...
export async function validateApiKey(rawKey: string): Promise<{
  ...
  // Soft format check: must look like an ECH key (min 20 chars, starts with 'ech_')
  // NOTE: We do NOT enforce the exact current prefix (ech_live_) because
  // keys generated by older versions (ech_xxx) must remain valid.
  // The SHA-256 hash is the authoritative validation, not the prefix.
  if (!rawKey || rawKey.length < 20 || !rawKey.startsWith('ech_')) {
    console.warn('[ApiKey] Rejected: invalid format (must start with ech_ and be >= 20 chars)', { prefix: rawKey?.substring(0, 20) || '(empty)' })
    return null
  }

  const keyHash = await hashKey(rawKey)
  console.log('[ApiKey] Looking up hash:', keyHash.substring(0, 16) + '...', 'for key prefix:', rawKey.substring(0, 17) + '...')
